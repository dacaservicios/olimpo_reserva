			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						<span class='oculto muestraId'>${resp.ID_CLIENTE}</span>
						<span class='oculto muestraNombre'></span>
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="las la-users-cog"></i> CLIENTE</div>
						<div class="row pt-3">
						...
						listado+=`</select>
							...
							<div class="form-group col-md-4">
								...
								<input name="documento" autocomplete="off" maxlength="15" type="tel" class="form-control p-1" placeholder="Ingrese el documento" value="${resp.NUMERO_DOCUMENTO}">
								<div class="vacio oculto">¡Campo obligatorio!</div>
						...
						<div class="row">
							...
							<div class="form-group col-md-6">
								...
								<input name="fechaNacimiento" maxlength="10" autocomplete="off" type="fecha" class="datepicker form-control" placeholder="Ingrese la fecha" value="${(resp.FECHA_NACIMIENTO===null)?'':moment(resp.FECHA_NACIMIENTO).format('DD-MM-YYYY')}">
							</div>
						</div>
						<div class="row">
							...
							<div class="form-group col-md-7">
								...
								<span id="imagenCliente" class="cursor">
...
function eventosCliente(objeto){
	...
	$('span#imagenCliente').on( 'click',function(){
		let imagen=`<img src="../imagenes/cliente/USU_`+verSesion()+`_`+objeto.nombreImagen+`">`;
			...
			if(resp.resultado){
				if(objeto.id>0){
					if(resp.info.IMAGEN!==null){
						...
						$("img.imagenClienteInicio").attr('src','/imagenes/cliente/CLI_'+resp.info.ID_CLIENTE+'_'+resp.info.IMAGEN);
					}

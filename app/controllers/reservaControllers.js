const {crearReserva,editarReserva,buscarReserva,listarReserva,estadoReserva,eliminarReserva,editarReservaDD,listarReservaDetalle,obtenerMensajeReserva} = require('../models/reservaModels');
const path = require('path');

const listar=(req, res)=>{
    const id =  req.params.id;
    const sesId=req.params.sesId;
    listarReserva(id,'reserva_cliente',sesId)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    }); 
}

const listarHora=(req, res)=>{
    const id =  req.params.id;
    const fecha =  req.params.fecha;
    const sesId=req.params.sesId;
    listarReservaDetalle(id,fecha,'verificaHora_reserva',sesId)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    }); 
}

const mensajeSucursal=(req, res)=>{
    const idSucursal =  req.params.idSucursal;
    const sesId=req.params.sesId;
    obtenerMensajeReserva(idSucursal,sesId)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    });
}

const buscar=(req, res)=>{
    const sesId =  req.params.sesId;
    const id =  req.params.id;
    buscarReserva(id,'reserva',sesId)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    }); 
}

const crear=(req, res)=>{
    let nombreImagen = null;
    if(req.archivo==1){
        const uploadedFile = req.files.imagen;
        const extension = uploadedFile.name.split('.').pop().toLowerCase();
        nombreImagen = 'RES_'+req.body.sesId+'_'+Date.now()+'.'+extension;
    }
    crearReserva(req.body, nombreImagen)
    .then(valor => {
        if(nombreImagen){
            const ruta = '../public/imagenes/reserva/'+nombreImagen;
            req.files.imagen.mv(path.join(__dirname,ruta), (err) => {
                if(err) console.error('Error al guardar imagen de reserva:', err);
            });
        }
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    });
}

const editar=(req, res)=>{
    const id=req.params.id;
    editarReserva(id,req.body)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    });
}

const editarDD=(req, res)=>{
    const id=req.params.id;
    editarReservaDD(id,req.body)
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    });
}

const eliminar=(req, res)=>{
    const id =  req.params.id;
    eliminarReserva(id,'reserva')
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    }); 
}

const estado=(req, res)=>{
    const id =  req.params.id;
    estadoReserva(id,'reserva')
    .then(valor => {
        res.json({
            valor : valor
        });
    })
    .catch(error => {
        res.status(400).json({
            error : {
                message:error.message,
                errno: error.errno,
                code : error.code
            }
        });
    }); 
}


module.exports = {
    listar,
    buscar,
    crear,
    editar,
    editarDD,
    estado,
    eliminar,
    listarHora,
    mensajeSucursal
}